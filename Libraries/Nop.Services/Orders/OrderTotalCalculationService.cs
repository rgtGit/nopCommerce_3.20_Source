...
namespace Nop.Services.Orders
{
    ...
    public partial class OrderTotalCalculationService : IOrderTotalCalculationService
    {
            ...
            decimal orderSubTotal, out Discount appliedDiscount)
        {
            ...
            /*debugOutput = "Account=" + customer.AccountNo;
            throw new ArgumentNullException(debugOutput, "R&R--SOOO DONE, YEAH!!!");*/
            ...
            foreach (CustomerRole role in customer.CustomerRoles) {
                if (role.Name.Contains("Discount "))
                {
                    roleName = role.Name;
                    break;
                    //throw new ArgumentNullException(role.Name, "R&R--SOOO DONE, YEAH!!!");
                }
            ...
            if (allDiscounts != null)
                foreach (var discount in allDiscounts)
                    ...
                    if (discount.Name == roleName) {
                        discountAmount = (decimal)((((float)orderSubTotal) * ((float)discount.DiscountPercentage)) / 100f);
                        break;
                    } 
...
*/
            ...
            out SortedDictionary<decimal, decimal> taxRates)
        {
            ...
            foreach (var shoppingCartItem in cart)
            {
                ...
                //throw new Exception ("R U OKAY, EMMA?   I REALLY MISS YOU!!!" + shoppingCartItem.)
        ...
        //    throw new Exception("At Bottom of procedure..." + subTotalWithoutDiscount);
        ...
        public virtual decimal? GetShoppingCartShippingTotal(IList<ShoppingCartItem> cart, decimal dropShipCharge, bool checkout)
        {
            //RGT 05.29.2018:  Consolidating all the nested Shipping Total calls into one.
            decimal shipSubTTL = decimal.Zero;
            decimal shippingTTL = decimal.Zero;

            Customer customer = cart.GetCustomer();

            #region "RGT-CODE NOT NEEDED"
            bool includingTax = false;
            switch (_workContext.TaxDisplayType)
            {
                case TaxDisplayType.ExcludingTax:
            ...
            //Add Shipping
            if (checkout) {
                var selectedShippingOption = _workContext.CurrentCustomer.GetAttribute<ShippingOption>(SystemCustomerAttributeNames.SelectedShippingOption, 
                    _storeContext.CurrentStore.Id);

                shipSubTTL = selectedShippingOption.Rate;
            }
            
            
            return shippingTTL = shipSubTTL;
        ...
        */
            ...
            out decimal taxRate, out Discount appliedDiscount)
        {
            ...
            //throw new Exception(shippingOption.Name);
            ...
            if (shippingOption != null)
            {
                ...
                shippingTotal = ttlQty * shippingOption.Rate; //  RGT --- AdjustShippingRate(shippingOption.Rate, cart, out appliedDiscount);
            ...
            else
            {
                  ...
                  
                {
                    ...
                    if (fixedRate.HasValue)
                    {
                        ...
                        shippingTotal = shippingTotal * cart.Count;
        ...
        */
        ...
        /// <param name="dropShipCharge">If new Shipping Address, this value will be greater than 0.00.</param>
        /// <returns>Shopping cart total;Null if shopping cart total couldn't be calculated now</returns>
        public virtual decimal? GetShoppingCartTotal(IList<ShoppingCartItem> cart, decimal dropShipCharge, bool checkout,
            bool ignoreRewardPonts = false, bool usePaymentMethodAdditionalFee = true)     
        {
            ...
            cartTotal =  Convert.ToDecimal(GetShoppingCartTotal(cart, dropShipCharge, checkout, out discountAmount, out appliedDiscount, 
            ...
            return cartTotal += +dropShipCharge;
        }
            ...
            bool ignoreRewardPonts = false, bool usePaymentMethodAdditionalFee = true)
        {
            ...
            decimal? shoppingCartShipping = GetShoppingCartShippingTotal(cart, dropShipCharge, checkout);
            ...
            if (orderTotal.HasValue)
            {
                ...
                return orderTotal += dropShipCharge;
            }
            else
                return null;
        }
