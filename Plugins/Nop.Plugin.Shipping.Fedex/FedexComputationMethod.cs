...
namespace Nop.Plugin.Shipping.Fedex
{
    ...
    public class FedexComputationMethod : BasePlugin, IShippingRateComputationMethod
    {
        ...
        private RateRequest CreateRateRequest(GetShippingOptionRequest getShippingOptionRequest, out Currency requestedShipmentCurrency)
        {
            ...
            switch (_fedexSettings.PackingType)
            {
                ...
                default:
                 ...
                 // RGT 06.25.2018:     New Packaging Type since we don't know receiving will pack each order.
                case PackingType.PackByTotalWeight:
                    SetShipmentByTotalWeight(request, getShippingOptionRequest, subTotalShipmentCurrency, requestedShipmentCurrency.CurrencyCode);
                    break;
            }
            
            //throw new Exception("Request set complete....");
            return request;
        }
        ...
        private void SetIndividualPackageLineItems(RateRequest request, GetShippingOptionRequest getShippingOptionRequest, decimal orderSubTotal, string currencyCode)
        {
            ...
            decimal weight = ConvertFromPrimaryMeasureWeight(_shippingService.GetShoppingCartTotalWeight(getShippingOptionRequest.Items), usedMeasureWeight);
            

            //throw new Exception(weight.ToString());
            if (length < 1)
                length = 1;
            if (height < 1)
                height = 1;
            ...
            if ((!IsPackageTooHeavy(weight)) && (!IsPackageTooLarge(length, height, width)))
            {
                ...
                request.RequestedShipment.PackageCount = "1";
                request.RequestedShipment.RequestedPackageLineItems = new RateServiceWebReference.RequestedPackageLineItem[1];
                request.RequestedShipment.RequestedPackageLineItems[0] = new RateServiceWebReference.RequestedPackageLineItem();
                ...
                request.RequestedShipment.RequestedPackageLineItems[0].Weight = new RateServiceWebReference.Weight(); 
                ...
                //weight;
                //throw new Exception(request.RequestedShipment.RequestedPackageLineItems[0].Weight.Value.ToString());
            ...
            else
            {
                ...
                decimal weight2 = weight / totalPackages;
        ...
        private void SetIndividualPackageLineItemsOneItemPerPackage(RateRequest request, GetShippingOptionRequest getShippingOptionRequest, decimal orderSubTotal, string currencyCode)
        {
            ...
            foreach (var sci in items)
            {
                ...
                decimal weight = ConvertFromPrimaryMeasureWeight(sci.Product.Weight, usedMeasureWeight);
                ...
                {
                    request.RequestedShipment.RequestedPackageLineItems[j] = new RateServiceWebReference.RequestedPackageLineItem();
                    request.RequestedShipment.RequestedPackageLineItems[j].GroupPackageCount = "1";
                    request.RequestedShipment.RequestedPackageLineItems[j].SequenceNumber = (j + 1).ToString(); // package sequence number            
                    request.RequestedShipment.RequestedPackageLineItems[j].Weight = new RateServiceWebReference.Weight(); // package weight
                    request.RequestedShipment.RequestedPackageLineItems[j].Weight.Units = RateServiceWebReference.WeightUnits.LB;
                    request.RequestedShipment.RequestedPackageLineItems[j].Weight.UnitsSpecified = true;
                    request.RequestedShipment.RequestedPackageLineItems[j].Weight.Value = weight;
                    request.RequestedShipment.RequestedPackageLineItems[j].Weight.ValueSpecified = true;

                    request.RequestedShipment.RequestedPackageLineItems[j].Dimensions = new RateServiceWebReference.Dimensions(); // package dimensions
                    request.RequestedShipment.RequestedPackageLineItems[j].Dimensions.Length = length.ToString();
                    request.RequestedShipment.RequestedPackageLineItems[j].Dimensions.Height =  height.ToString();
                    request.RequestedShipment.RequestedPackageLineItems[j].Dimensions.Width = width.ToString();
                    request.RequestedShipment.RequestedPackageLineItems[j].Dimensions.Units = RateServiceWebReference.LinearUnits.IN;
                    request.RequestedShipment.RequestedPackageLineItems[j].Dimensions.UnitsSpecified = true;
                                                                        
                    request.RequestedShipment.RequestedPackageLineItems[j].InsuredValue = new Money(); // insured value
                    request.RequestedShipment.RequestedPackageLineItems[j].InsuredValue.Amount = sci.Product.Price;
                    request.RequestedShipment.RequestedPackageLineItems[j].InsuredValue.Currency = currencyCode;
        ...
        private void SetShipmentByTotalWeight(RateRequest request, GetShippingOptionRequest getShippingOptionRequest, decimal orderSubTotal, string currencyCode)
        {
            // RGT 06.25.2018:  Since we don't know how Receiving will package each order, this procedure was added to
            // the FedEx Plugin to get shipping rates based on total cart weight.
            ...
            foreach (var sci in items)
            {
                weight += (ConvertFromPrimaryMeasureWeight(sci.Product.Weight, usedMeasureWeight) * sci.Quantity);
                //throw new Exception(weight.ToString());
            }
            ...
            // No Dimensions will passed to the FedEx API.
            request.RequestedShipment.RequestedPackageLineItems[0].Dimensions = new RateServiceWebReference.Dimensions(); // package dimensions
            request.RequestedShipment.RequestedPackageLineItems[0].Dimensions.UnitsSpecified = false;


            request.RequestedShipment.RequestedPackageLineItems[0].InsuredValue = new Money(); // insured value
            request.RequestedShipment.RequestedPackageLineItems[0].InsuredValue.AmountSpecified = false;
            ...
            {   
                ...
                if (!serviceName.Equals("UNKNOWN"))
                {
                    ...
                    foreach (RatedShipmentDetail shipmentDetail in rateDetail.RatedShipmentDetails)
                    {
                        ...
                        /*
                        if (_fedexSettings.ApplyDiscounts & shipmentDetail.ShipmentRateDetail.RateType == ReturnedRateType.PAYOR_ACCOUNT_PACKAGE)
                        {
                        ...
                        else if (shipmentDetail.ShipmentRateDetail.RateType == ReturnedRateType.PREFERRED_LIST_PACKAGE) // Get List Rates (not discount rates)
                        {
                            decimal amount = ConvertChargeToPrimaryCurrency(shipmentDetail.ShipmentRateDetail.TotalNetCharge, requestedShipmentCurrency);
                            throw new Exception("Amount=" + shippingOption.Rate.ToString());
                            shippingOption.Rate = amount + _fedexSettings.AdditionalHandlingCharge;
                            break;
                        }
                ...
                shippingOption.DeliveryTimeStamp = rateDetail.DeliveryTimestamp.ToShortDateString();
        ...
        {
            //throw new Exception(getShippingOptionRequest.GetType());
