...
namespace Nop.Admin.Controllers
{
    ...
    public partial class ShippingController : BaseNopController
	{
        ...
        [HttpPost, GridAction(EnableCustomBinding = true)]
        public ActionResult DropShip(GridCommand command)
        {
            if (!_permissionService.Authorize(StandardPermissionProvider.ManageShippingSettings))
                return AccessDeniedView();

            var shippingDropModel = _shippingService.GetAllDropShipSettings()
                .Select(x => x.ToModel())
                .ForCommand(command)
                .ToList();
            var model = new GridModel<ShippingDropModel>
            {
                Data = shippingDropModel,
                Total = shippingDropModel.Count
            };
        ...
        public ActionResult EditDropShip(int id)
        {
            if (!_permissionService.Authorize(StandardPermissionProvider.ManageShippingSettings))
                return AccessDeniedView();

            var sm = _shippingService.GetShippingDropById(id);
            if (sm == null)
                //No shipping method found with the specified id
                return RedirectToAction("ShipDrop");
            ...
            AddLocales(_languageService, model.Locales, (locale, languageId) =>
            {
                locale.MinimumCharge = sm.GetLocalized(x => x.MinimumCharge, languageId, false, false);
                locale.PercentageOfSubTotal = sm.GetLocalized(x => x.PercentageOfSubTotal, languageId, false, false);
            });
            */
            return View(model);
        }
        ...
        [HttpPost, ParameterBasedOnFormName("save-continue", "continueEditing")]
        public ActionResult EditDropShip(ShippingDropModel model, bool continueEditing)
        {
            
            if (!_permissionService.Authorize(StandardPermissionProvider.ManageShippingSettings))
                return AccessDeniedView();

            var sm = _shippingService.GetShippingDropById(model.Id);
            if (sm == null)
                //No shipping method found with the specified id
                return RedirectToAction("EditDropShip");

            if (ModelState.IsValid)
            {
                sm = model.ToEntity(sm);
                _shippingService.UpdateShippingDrop(sm);

                //locales
                //*** RGT 10.23.2017-- not needed...
                /*UpdateLocales(sm, model);
                SuccessNotification(_localizationService.GetResource("Admin.Configuration.Shipping.Methods.Updated"));*/

                return continueEditing ? RedirectToAction("EditDrop", sm.Id) : RedirectToAction("DropShip");
            }
